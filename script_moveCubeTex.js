// 動的キューブ環境マッピング
//
// まず、フレームバッファにキューブマップを動的に生成するために背景を 6 回、トーラスを 6 回レンダリング
// 次に、最終的なシーンをレンダリングするために背景を 1 回とトーラスを 6 回、さらに球体を 1 回レンダリング


// // フレームバッファをオブジェクトとして生成する関数(キューブマップ仕様)に変更！！
// // ３つめの引数できた！！
// function create_framebuffer(width, height, target){

//フレームバッファのサイズより大きくないと貼り付けたとき隙間できる！！
...
onload = function(){
	...
	c.width = 700;//フレームバッファのサイズより大きくないと貼り付けたとき隙間できる！！
	...
	var fBuffer = create_framebuffer(fBufferWidth, fBufferHeight, cubeTarget);//３つめの引数！！！
