// sample_025
//フレームバッファで先に絵をかいて、canvasでその絵をキューブ側面に貼り付ける！


// 372//フレームバッファー用テクスチャにカラー用のメモリ領域を確保
// は最後の引数はnullにしておく。テクスチャにはそのサイズやフォーマットだけが設定され、中身は空っぽの状態になります。要は、この空の領域にオフスクリーンレンダリングを行なうわけです。ですから、重要なのはテクスチャのサイズを正しく指定すること、そして最後の引数を必ずnullにすることです。
...
onload = function(){
	...
	(function(){
        ...
        // モデル座標変換行列の生成
        m.identity(mMatrix);
        // m.translate(mMatrix, [tr[0], tr[1], tr[2]], mMatrix);kooooooooooooooooo
        m.rotate(mMatrix, rad2, [0, 1, 1], mMatrix);
        m.multiply(tmpMatrix, mMatrix, mvpMatrix);
        m.inverse(mMatrix, invMatrix);
	...
	function create_framebuffer(width, height){
        ...
        // フレームバッファにテクスチャを関連付ける//ここはキューブマッピングではレンダリングが行われる段階でやる！
